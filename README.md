...
- `摆谱`：载入初始棋盘，可在程序棋盘上点击棋子走子。
- `棋谱库`：打开棋谱库入口。
- `打分图`：在右侧显示每步的开局库分数摘要。
- `导航`：默认打开，用中文象棋记谱显示历史步数和开局库分支走法，并可跳到第一步、上一步、下一步、最后一步。也可以勾选红方或黑方电脑走棋，并选择时间优先或深度优先。开局库默认读取项目目录下的 `book.xqb`。
- `PV分析`：计算时显示实时 PV 分析。
- `设置`：打开设置窗口，里面有校准、自动走棋、头像识别和下一局识别。
...
联机监控默认不再用头像绿色圈判断。点 `开始监控` 后，程序会先记住当前棋盘，等待对方走棋；识别到棋盘变化并稳定后，才开始为我方计算和走棋。

在 `连线 -> 设置` 里可以设置：

- `旧方式：用头像判断轮到我`：默认关闭，只有你确实想用头像绿色圈时再打开。
- `识别头像位置`：旧方式需要先框选头像绿色倒计时区域。

新方式更适合联机：对方走完，棋盘变化，程序再计算。
